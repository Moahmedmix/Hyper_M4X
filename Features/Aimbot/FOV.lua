--[[
    ...
    - Wall check (Raycast)
    - Auto Shoot for mobile (3 methods)
--]]
...
Aimbot.Settings = {
    Enabled = false, FOV = 150, Smoothness = 0.1, AimPart = "Head",
    TeamCheck = false, ShowFOV = true,
    FOVColor = Color3.fromRGB(255,255,255), LockedColor = Color3.fromRGB(255,0,0),
    FOVThickness = 2, FOVNumSides = 64,
    Prediction = 0, VisibleCheck = false, WallCheck = true,
    AutoShoot = true, ShootDelay = 0.1,
}
...
function Aimbot:Init(tab, library, flags)
    ...
    self.Tab = tab; self.Library = library; self.Flags = flags; self.Connection = nil

    local ss = Camera.ViewportSize
    FOVCircle = Drawing.new("Circle")
    FOVCircle.Visible = false; FOVCircle.Color = Aimbot.Settings.FOVColor
    FOVCircle.Thickness = Aimbot.Settings.FOVThickness; FOVCircle.NumSides = Aimbot.Settings.FOVNumSides
    FOVCircle.Radius = Aimbot.Settings.FOV; FOVCircle.Filled = false
    FOVCircle.Position = Vector2.new(ss.X/2, ss.Y/2); FOVCircle.ZIndex = 10

    Camera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
        local s = Camera.ViewportSize; FOVCircle.Position = Vector2.new(s.X/2, s.Y/2)
    end)

    self:BuildUI(); self:StartLoop()
    return self
end
...
function Aimbot:BuildUI()
    ...
    mainSec:Slider({Title="Shoot Delay",Step=0.01,Value={Min=0,Max=0.5,Default=0.1},Callback=function(v)Aimbot.Settings.ShootDelay=v end})

    local circleSec = self.Tab:Section({Title="Circle",Icon="circle",Opened=true})
    ...
    colorSec:Colorpicker({Title="Normal",Default=Aimbot.Settings.FOVColor,Transparency=0,Callback=function(c)Aimbot.Settings.FOVColor=c end})
    colorSec:Colorpicker({Title="Locked",Default=Aimbot.Settings.LockedColor,Transparency=0,Callback=function(c)Aimbot.Settings.LockedColor=c end})

    local aimSec = self.Tab:Section({Title="Aim",Icon="target",Opened=true})
    aimSec:Slider({Title="Smoothness",Step=0.01,Value={Min=0,Max=1,Default=0.1},Callback=function(v)Aimbot.Settings.Smoothness=v end})
    aimSec:Slider({Title="Prediction",Step=0.01,Value={Min=0,Max=0.5,Default=0},Callback=function(v)Aimbot.Settings.Prediction=v end})
    ...
    aimSec:Toggle({Title="Wall Check",Value=true,Callback=function(s)Aimbot.Settings.WallCheck=s end})
end
...
function Aimbot:Update()
    ...
    if Aimbot.Settings.ShowFOV then
        FOVCircle.Visible = true; FOVCircle.Radius = Aimbot.Settings.FOV
        FOVCircle.Thickness = Aimbot.Settings.FOVThickness; FOVCircle.NumSides = Aimbot.Settings.FOVNumSides
...
function Aimbot:IsVisible(player)
    ...
    if result then return result.Instance:IsDescendantOf(char) end
    return true
end
...
function Aimbot:GetClosest(center)
    local closest = nil; local shortest = Aimbot.Settings.FOV
...
function Aimbot:AutoShoot()
    ...
    -- Method 1: Direct tool fire (best for mobile)
    pcall(function()
