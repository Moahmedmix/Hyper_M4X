...
function TP:Init(tab, library, flags)
    ...
    Sec:Dropdown({ Title = "Select Player", Values = GetPlayers(), Value = "", Callback = function(v) TP.Settings.Target = v end })
    ...
    Sec:Button({ Title = "Teleport", Callback = function()
        ...
        local p = Players:FindFirstChild(TP.Settings.Target)
    ...
    Sec:Button({ Title = "Save Position", Callback = function() local c = LocalPlayer.Character; if c and c:FindFirstChild("HumanoidRootPart") then TP.Settings.Saved = c.HumanoidRootPart.CFrame; library:Notify({ Title = "TP", Description = "Saved!", Duration = 2 }) end end })
    Sec:Button({ Title = "Load Position", Callback = function() if TP.Settings.Saved then DoTP(TP.Settings.Saved) end end })
    Sec:Button({ Title = "TP Up 50", Callback = function() local r = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart"); if r then DoTP(r.Position + Vector3.new(0, 50, 0)) end end })
    Sec:Button({ Title = "TP Down 50", Callback = function() local r = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart"); if r then DoTP(r.Position - Vector3.new(0, 50, 0)) end end })
