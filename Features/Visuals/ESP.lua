...
ESP.Settings = {
    ...
    -- Head Dot
    HDot = true,
    HDotColor = Color3.fromRGB(255, 255, 255),
    HDotSize = 7,
    HDotGlow = true,
    HDotGlowAlpha = 0.7,
...
-- UTILITY
-- =============================================
function ESP:FindRoot(char)
    for _, n in ipairs(RootNames) do local p = char:FindFirstChild(n) if p and p:IsA("BasePart") then return p end end
    for _, p in ipairs(char:GetChildren()) do if p:IsA("BasePart") and p.Name ~= "Head" then return p end end
    return nil
end

function ESP:FindHead(char)
    for _, n in ipairs(HeadNames) do local p = char:FindFirstChild(n) if p and p:IsA("BasePart") then return p end end
    local hi, hy = nil, -math.huge
    for _, p in ipairs(char:GetChildren()) do if p:IsA("BasePart") then local y = p.Position.Y if y > hy then hy = y; hi = p end end end
    return hi
end
...
function ESP:GetColor(player)
    ...
    if ESP.Settings.TeamColor and player.Team and LocalPlayer.Team then
        if player.Team == LocalPlayer.Team then return Color3.fromRGB(50, 255, 50) end
...
function ESP:Update()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if ESP.Settings.TeamCheck and p.Team == myTeam then ESP:CleanPlayer(p) continue end
        ...
        -- ============ CORNER BOX ============
        b[1].From, b[1].To = Vector2.new(x, y), Vector2.new(x + cl, y)
        b[1].Color, b[1].Thickness, b[1].Visible = color, ct, true
        b[2].From, b[2].To = Vector2.new(x, y), Vector2.new(x, y + cl)
        b[2].Color, b[2].Thickness, b[2].Visible = color, ct, true
        b[3].From, b[3].To = Vector2.new(x + bw - cl, y), Vector2.new(x + bw, y)
        b[3].Color, b[3].Thickness, b[3].Visible = color, ct, true
        b[4].From, b[4].To = Vector2.new(x + bw, y), Vector2.new(x + bw, y + cl)
        b[4].Color, b[4].Thickness, b[4].Visible = color, ct, true
        b[5].From, b[5].To = Vector2.new(x, y + bh), Vector2.new(x + cl, y + bh)
        b[5].Color, b[5].Thickness, b[5].Visible = color, ct, true
        b[6].From, b[6].To = Vector2.new(x, y + bh - cl), Vector2.new(x, y + bh)
        b[6].Color, b[6].Thickness, b[6].Visible = color, ct, true
        b[7].From, b[7].To = Vector2.new(x + bw - cl, y + bh), Vector2.new(x + bw, y + bh)
        b[7].Color, b[7].Thickness, b[7].Visible = color, ct, true
        b[8].From, b[8].To = Vector2.new(x + bw, y + bh - cl), Vector2.new(x + bw, y + bh)
        b[8].Color, b[8].Thickness, b[8].Visible = color, ct, true

        -- ============ NAME ============
        if ESP.Settings.Name then
            b.Name.Text = p.DisplayName
            b.Name.Color = ESP.Settings.NameColor
            b.Name.Size = ESP.Settings.NameSize
            b.Name.Position = Vector2.new(rsp.X, y - 16)
            b.Name.Visible = true
            b.Name.Outline = ESP.Settings.NameOutline
        ...
        if ESP.Settings.Dist then
            ...
            b.Dist.Color = ESP.Settings.DistColor
            b.Dist.Size = ESP.Settings.DistSize
            b.Dist.Position = Vector2.new(rsp.X, y + bh + 14)
            b.Dist.Visible = true
        ...
        if ESP.Settings.HP and hum then
            ...
            b.HPBg.Size = Vector2.new(bw2, bh)
            b.HPBg.Position = Vector2.new(hpx, y)
            b.HPBg.Color = ESP.Settings.HPBG
            b.HPBg.Visible = true
            b.HPFill.Size = Vector2.new(bw2, bh * pct)
            b.HPFill.Position = Vector2.new(hpx, y + bh * (1 - pct))
            b.HPFill.Color = hc
        ...
        if ESP.Settings.HDot then
            ...
            b.HDot.Position = Vector2.new(hcsp.X, hcsp.Y)
            b.HDot.Radius = ESP.Settings.HDotSize
            b.HDot.Color = ESP.Settings.HDotColor
            b.HDot.Visible = true
            if ESP.Settings.HDotGlow then
                b.HDotGlow.Position = Vector2.new(hcsp.X, hcsp.Y)
                b.HDotGlow.Radius = ESP.Settings.HDotSize + 4
                b.HDotGlow.Color = ESP.Settings.HDotColor
                b.HDotGlow.Transparency = ESP.Settings.HDotGlowAlpha
                b.HDotGlow.Visible = true
            end
        ...
        if ESP.Settings.Tracer then
            ...
            b.Tracer.From = Vector2.new(sx, sy)
            b.Tracer.To = Vector2.new(rsp.X, rsp.Y)
            b.Tracer.Color = ESP.Settings.TracerColor
            b.Tracer.Thickness = ESP.Settings.TracerThick
            b.Tracer.Visible = true
        else b.Tracer.Visible = false end

        -- ============ SNAPLINE ============
        if ESP.Settings.Snap then
            b.Snap.From = Vector2.new(cx, cy)
            b.Snap.To = Vector2.new(rsp.X, rsp.Y)
            b.Snap.Color = ESP.Settings.SnapColor
            b.Snap.Thickness = ESP.Settings.SnapThick
            b.Snap.Visible = true
        ...
        if ESP.Settings.Weapon then
            ...
            for _, tool in ipairs(c:GetChildren()) do
                if tool:IsA("Tool") and tool:FindFirstChild("Handle") then wep = tool.Name; break end
            end
            if wep then
                b.Weapon.Text = "[" .. wep .. "]"
                b.Weapon.Color = ESP.Settings.WeaponColor
                b.Weapon.Size = 11
                b.Weapon.Position = Vector2.new(rsp.X, y + bh + 28)
        ...
        if ESP.Settings.Skeleton then
            ...
            for _, bone in ipairs(SkeletonBones) do
                local p1 = c:FindFirstChild(bone[1])
                local p2 = c:FindFirstChild(bone[2])
                if p1 and p2 then
                    ...
                    if sp1.Z > 0 and sp2.Z > 0 then
                        b[skelIdx].From = Vector2.new(sp1.X, sp1.Y)
                        b[skelIdx].To = Vector2.new(sp2.X, sp2.Y)
                        b[skelIdx].Color = ESP.Settings.SkeletonColor
                        b[skelIdx].Thickness = ESP.Settings.SkeletonThick
                        b[skelIdx].Visible = true
                        skelIdx = skelIdx + 1
                    end
                end
            end
        ...
        if ESP.Settings.InfoPanel then
            ...
            b.Info.Size = 11
            b.Info.Position = Vector2.new(rsp.X + bw + 60, rsp.Y)
            b.Info.Visible = true
    ...
    for p in pairs(ESP.Boxes) do
        if not processed[p] then ESP:CleanPlayer(p) end
    end
end
...
function ESP:Init(tab, library, flags)
    ...
    SecDot:Toggle({ Title = "Glow Effect", Value = true, Callback = function(v) ESP.Settings.HDotGlow = v end })
    ...
    SecExtra:Toggle({ Title = "Weapon", Value = false, Callbac
